...
| 诚实性 | 无据陈述率 / 健康禁词泄漏 | **0 / 0** | E2 E31–33 |
...
| 编排延迟 | p50 / p95（MockLLM） | **4.6 / 5.6 ms** | — |
...
| 项 | 为什么 |
| --- | --- |
| E21 声学情境分类 `macro-F1` | `data/priors/catmeows_stats.json` **全是占位值**（`is_placeholder: true`）。用它算出的分数测的是「拟合占位值的程度」 |
